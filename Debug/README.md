Various debug codes

- Less Restrictive Crash Debugger || loosens the reigns on what will trigger the crash debugger.

- Go To LAN Instead Of Debug || goes to the LAN menu when pressing the Debug menu in Hack Pack, only works in Dolphin if the BBA is enabled.

- Pause When Game Start || pauses when the game starts

- Allow Machine Select In CT || allows you to select your machine in CT

- Show JObjs In Top Ride || shows the hidden J Objs in Top Ride

- Toggleable G Obj Displays || shows the G Obj Displays